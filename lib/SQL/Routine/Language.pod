...
SQL::Routine manages SQL concepts that we arrange into 4 main categories for
convenience: 'blueprints', 'tools', 'sites', 'circumventions'.

The first of these, 'blueprints', is the most important; it describes
completely the composition and behaviour of your database schemas and
application-level routines that talk to them.  For all intents and purposes, it
is part of your core application code and would get distributed as such; like
your core application code, everyone's installation will be identical.  We
describe most model structures in 'blueprints', including database tables (with
...
at a site), structures belonging to each of the 4 categories would be stored
...
implicit container_type of SCALAR.
	...
	SCALAR - a scalar variable, such as a boolean or number or cstring or bstring
	ERROR  - an error message object (a message key, named list of values)
	CONN   - a database connection context
	CURSOR - a database cursor variable (and context when open)
	...
	ENVI   - a db/app environment context; usually just one (conceptually), which is global
	TRAN   - a database transaction context
	RECORD - a row variable; a named list of scalars, like a Perl hash
	ARRAY  - an ordered list of scalars; or of rows; like SQL:2003 'table' var type
...
=head2 standard_routine, standard_routine_arg
...
Beneath each routine name is up to 4 kinds of information, having the following
labels; for brevity, a label won't be shown if its value would be 'none':
	...
	args: what arguments the routine takes, if any
...
routine, client or database side, usually with 'routine_stmt' Nodes.
	...
	CATALOG_OPEN() 
		...
		args:
			...
			- LOGIN_USER - SCALAR.cstr - Opt: A runtime-provided username to connect with.
			- LOGIN_PASS - SCALAR.cstr - Opt: A runtime-provided password to connect with.
		effects:
			- The CONN_CX changes from an empty variable to active db connection handle.
		throws:
			- SRTX_NO_ENVI_LOAD_FAILED, SRTX_ENVI_EXEC_FAILED,
			SRTX_NO_CONN_SERVER_ABSENT, SRTX_NO_CONN_BAD_AUTH,
			SRTX_NO_CONN_ACTIVE_LOST
	...
	CATALOG_CLOSE() 
		...
		effects:
			- The context provided by CONN_CX is destroyed.
		throws:
			- SRTX_NO_CONN_ACTIVE_LOST
...
with 'routine_stmt' Nodes.
	...
	CURSOR_OPEN() 
		purpose:
			- Opens a select cursor for reading from (or performs a select if in right context).
		args:
			- CONN_CX - CONN - An open database connection to provide our action context.
	...
	INSERT() 
		purpose:
			- Inserts a row into a table/view.
		args:
			- CONN_CX - CONN - An open database connection to provide our action context.
...
view, client or db side, usually with 'routine_expr' or 'view_expr' Nodes.
	...
	CAST() 
		purpose:
			- Cast a single child expression into a different data-type specified by a domain node.
		args:
			- CAST_TARGET - SRT_NODE:domain - The data type to cast the operand as.
	...
	update, delete, commit, rollback] to be an SRTN.
...
variables (or literals) whose container_type is SCALAR.  The 1999 SQL standard
indicates 5 classes of simple (Predefined) data types: Numeric, String,
Boolean, Datetime, Interval; all of the data types in those classes are
represented by one of the following enumerated values.  The 2003 SQL standard
adds a 6th one, XML, but we will ignore that one for the moment.
...
other 4 pseudo-Nodes: blueprints, tools, sites, circumventions.
...
Exclusive Attribute Groups List:
	...
	privilege_on - schema,domain,sequence,table,view,routine - MA
...
simple_data_type, the latter of which SQL::Routine recognizes in code. This
...
Attribute List:
	...
	id - NODE_ID - unique identifier for each Node within this Node Type
	schema - N.schema - PP - the schema that this domain is in, if any
	application - N.application - PP - the app-space that this domain is in, if any
...
Attribute List:
	...
	application - N.application - PP - the app-space that this sequence generator is in
...
Attribute List:
	...
	application - N.application - PP - the app-space that this table is in
...
Attribute List:
	...
	application - N.application - PP - the app-space that this view is in, if any
	routine - N.routine - PP - parent routine of this one, if any
...
Note: While SQL::Routine supports the SQL:2003/1999 "with" syntax, that
...
Attribute List:
	...
	start_literal - L.misc - if start_src_col is being compared to a literal, then store the literal value here
	start_view_arg - N.view_arg - when view is a "with" sub-query, use value of an argument to the view
	start_routine_arg - N.routine_arg - use value of an arg to the current routine
	start_routine_var - N.routine_var - use value of a variable declared in the current routine
...
Exclusive Attribute Groups List:
	...
	start_val - start_literal,start_view_arg,start_routine_arg,start_routine_var - MA
...
Attribute List:
	...
	set_result_col - N.view_col - the view interface/result column we are defining the value of
	set_src_col - N.view_src_col - this is the column we are setting the value of in insert or update op
	...
	domain - N.domain - cast or declare curr literal expr into this data type / domain
	valf_src_col - N.view_src_col - use the value of a local view source column
	valf_result_col - N.view_col - match a result column; used mainly with MATCH view, or in 'having','order' sql elsewhere
	valf_p_view_arg - N.view_arg - when view is a "with" sub-query, use value of an argument to the view
...
Local Attribute Dependencies List:
	...
	view_part:
		set_result_col - RESULT - MA
		set_src_col - SET - MA
		call_src_arg - FROM - MA
	cont_type:
	#	valf_literal,valf_src_col,valf_result_col,valf_p_view_arg,
	#		valf_p_routine_arg,valf_p_routine_var,valf_seq_next,valf_call_view,
	#		valf_call_sroutine,valf_call_uroutine - SCALAR,ERROR,CONN,CURSOR - MA
	#	valf_p_routine_arg,valf_p_routine_var - ERROR,CONN,CURSOR - MA
	valf_literal:
		domain - MA
	valf_call_uroutine:
		catalog_link
...
Attribute List:
	...
	application - N.application - PP - the app-space that this routine is in, if any
...
Attribute List:
	...
	of 'assign_dest_[arg|var]' is set, the action is an assignment of an
	...
	Meanwhile, all exceptions are thrown out to the application using SRT.

Exclusive Attribute Groups List:

	stmt_type - block_routine,assign_dest_arg,assign_dest_var,call_sroutine,call_uroutine - MA
...
Attribute List:
	...
	actn_domain - N.domain - if act-on sroutine arg should be a domain Node, this is it
...
Local Attribute Dependencies List:
	...
	cont_type:
	#	valf_literal,valf_p_routine_arg,valf_p_routine_var,valf_seq_next,
	#		valf_call_sroutine,valf_call_uroutine - SCALAR,ERROR,CONN,CURSOR - MA
	#	valf_p_routine_arg,valf_p_routine_var - ERROR,CONN,CURSOR - MA
