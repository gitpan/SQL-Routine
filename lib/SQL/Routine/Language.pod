...
SQL::Routine's 2004-05-12 release, domains are under 'blueprints'.  We
...
take a SQL::Routine "command" or "routine" Node (or an object wrapping one
...
These are the basic types that a routine can be, since SQL::Routine
...
These are the basic types of SQL::Routine Commands that an application can
...
SQL::Routine command against a database or other Result context.
...
This type of Node describes a SQL::Routine pseudo-routine called a command,
...
Attribute List:
	...
	command_type - E.command_type - MA - what SQL::Routine recognized command to run
