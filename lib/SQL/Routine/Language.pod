...
B<Ancestor Attribute Correlation List>, B<Child Quantity List>, B<Distinct Child
Groups List>.
...
that any "SI" and "WR" and "MA" attributes are populated.  All "Attribute List"
...
row_domain attribute points to.  Each list element is in this format:
	...
	Note that 'S' is a no-op, displayed for easier documentation reading
	Note that if a Node at the end of the above process has a WR field, it is implicitly followed.
	If 'C' is used, search text must be array of degree N+1 where N is the number of 'C'.
	If 'C' is not used, search text is not an array / has one scalar value.
