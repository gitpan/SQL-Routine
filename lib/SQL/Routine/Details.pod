...
to a Node in this Container, which includes creating or deleting a Node.
...
processed by Node.set_attributes() if it is provided; a Node id can also be
provided this way, or it will be generated if that is allowed.  If ATTRS is
defined but not a Hash ref, then this method will build a new one having a
