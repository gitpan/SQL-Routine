...
=head1 CONTENT OF libSQLRT.h
	...
	typedef struct {
		...
		union { /* when 'container' is null, use 'id'; when it isn't null, use 'ref' */
			sqlrt_dt_scale id ; /* id that has to be resolved to point to a Node */
			sqlrt_dt_node* ref; /* pointer to a Node */
		...
		sqlrt_dt_node_atnm  pp_node_atnm; /* name of at_nodes attribute which is our primary parent */
		sqlrt_dt_container* container   ; /* (ref) what Container this Node lives in, if any */
		sqlrt_dt_node_dll*  child_nodes ; /* (list of refs) recipr to Nodes whose at_nodes ref to us */
	} sqlrt_dt_node;
	...
	typedef struct {
		sqlrt_dt_node_dll** all_nodes; /* (array of list of refs) all Nodes in this Container */
